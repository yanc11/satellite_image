...
l = len(filenames)
fdic = {'1':1,'2':2,'13':3,'16':4,'18':5,'21':6,'7':7}
...
for i in range(l):
	...
	image = ''
	try:
		image = caffe.io.load_image('../../imgs/4class/%s.png'%filenames[i])
	except:
		bad.write('%s\n'%filenames[i])
		continue
	...
	out3.write(filenames[i].split('_')[0])
	s_fe = [0 for kk in range(4096)]
	for j in range(10):
		feature = net.blobs['fc6'].data[j]
		if j ==4:
			out2.write(filenames[i].split('_')[0])
